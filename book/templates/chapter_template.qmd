...
10. Exactly one named persona (Diego/completions, Sarah/intervention,
...
TikZ -> PDF/SVG pattern. Two shapes exist in figures/diagrams/generate_diagrams.py:

- A straight vertical chain with no branching: add labels to
  THEORY_DIAGRAMS (not DIAGRAMS) — see chapter_01, 02, 04, 05, 09, 11,
  and 12's main pipeline for worked examples, including multi-line
  labels (a step name plus a smaller annotation, e.g. "Extraction" /
  "Chapter 1 -- pdfplumber"). THEORY_DIAGRAMS' generic builder still
  only draws a straight chain — it has no branch/fork support.
- Anything that branches — a yes/no decision, an exception exit, a
  fan-out into independent parallel outputs, or independent side-by-side
  components with no flow between them — needs a hand-built one-off
  TikZ block instead (not a THEORY_DIAGRAMS entry). See DAILY_LOOP_TEX
  (Ch 13, phase-grouped chain with 2 exception exits), ROUTING_TEX
  (Ch 10, symmetric two-column decision, neither branch an exception),
  QUALITY_GATE_TEX (Ch 6, main chain + 1 exception exit), DUAL_INDEX_TEX
  (Ch 8, fan-out fork, no decision), and SATELLITES_TEX (Ch 12, two
  independent boxes, no arrows at all) for five different worked
  shapes — match the diagram's shape to what the content actually is,
  don't force it into whichever template is closest.
