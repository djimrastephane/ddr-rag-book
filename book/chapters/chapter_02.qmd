...
![](../figures/diagrams/pipeline_ch02.pdf){width="150" fig-align="center"}
:::

::: {.chapter-status}
Progress `██░░░░░░░░░░░` **2 / 13** &nbsp;·&nbsp; **Estimated time:** 30–45 min &nbsp;·&nbsp; **Difficulty:** 🟢 Beginner
:::
...
- Explain why oilfield shorthand breaks naive text search and NLP
  (natural language processing) tools alike.
...
Carlos, the completions engineer, wants next chapter's search tool to
...
`BHA` — nine times. A search tool that only knows the literal words
...
1. **Word boundaries.** `str.replace("MD", ...)` will happily mangle a
   ...
   with a *regular expression* — a text-matching pattern, or *regex* for
   short — `\b...\b`, not raw substring replacement.
...
::: {.content-visible when-format="html"}
::: {.pipeline-diagram}
![](../figures/diagrams/theory_ch02_expand_light.svg){.diagram-light width="280"}
![](../figures/diagrams/theory_ch02_expand_dark.svg){.diagram-dark width="280"}
:::
:::

::: {.content-visible when-format="pdf"}
![](../figures/diagrams/theory_ch02_expand.pdf){width="280" fig-align="center"}
:::
...
**Try it yourself:** Help Carlos get his answer — run the expander against
...
and think carefully about whether blindly expanding a two-letter
abbreviation like `PV` is actually safe against this archive's real
text, or whether
...
- Cleaning text isn't optional busywork — the expanded text is what
  Chapter 3's keyword search matches against. (Chapter 4's semantic search
  works from the raw text instead; its Field Notes measure exactly how
  much expansion does and doesn't help there.)
