...
![](../figures/diagrams/pipeline_ch01.pdf){width="160" fig-align="center"}
:::

::: {.chapter-status}
Progress `█░░░░░░░░░░░░` **1 / 13** &nbsp;·&nbsp; **Estimated time:** 45–60 min &nbsp;·&nbsp; **Difficulty:** 🟢 Beginner
:::
...
Before we can search this archive, summarise it, or ask it questions, we
...
::: {.content-visible when-format="html"}
::: {.pipeline-diagram}
![](../figures/diagrams/theory_ch01_extract_light.svg){.diagram-light width="260"}
![](../figures/diagrams/theory_ch01_extract_dark.svg){.diagram-dark width="260"}
:::
:::

::: {.content-visible when-format="pdf"}
![](../figures/diagrams/theory_ch01_extract.pdf){width="260" fig-align="center"}
:::
...
than a simple narrative — this one report alone holds seven distinct data
tables (casing, mud, drill bits, pumps, BHA, survey data, consumables),
plus a header and a narrative time breakdown — and all of it came back as
one ordinary block of text you can now search, print, or save.
...
`enumerate()` is what hands back that running `page_number` alongside
each page as the loop repeats — `start=1` just means "count from 1, not
0," so the numbers it produces read the way a person would number pages,
not the way Python numbers list positions internally. This same
`enumerate(..., start=1)` pattern shows up throughout the book any time
something needs to be numbered for a human reader.
:::
...
`None` — Python's way of marking "nothing here," not the same thing as an
empty string — and every downstream step in this book expects a string,
not `None`. This is a safety check, the code equivalent of confirming a
valve position before proceeding instead of assuming it: assume real
data is messier than the happy path, and this pattern shows up throughout
the book.

::: {.callout-note title="If this page were scanned instead"}
None of Part I's ten reports are scanned — but if one were,
`page.extract_text()` wouldn't raise an error or hand back garbled
characters. It would simply return `None`, because there's no character
data on the page to read, only a picture of one. That's exactly the case
the `or ""` above guards against. Chapter 6 covers scanned reports
properly: detecting them automatically and recovering their text with
OCR.
:::
...
stuck` — real language from a real rig report, not a paraphrase. You no
longer need to open that PDF and copy this line out by hand — and every
capability the rest of this book adds, from search to cited answers,
starts from this one extraction step.

::: {.callout-tip title="Peek ahead"}
Right now, you've extracted text from one report. By Chapter 5 you'll
ask this archive natural-language questions and get cited answers back.
By Chapter 13, the system will pick up a brand-new report on its own,
without anyone re-running this script by hand. Every one of those later
chapters still starts here.
:::
...
the same horizontal line of the page — get run together into one line of
