...
An engineer searches "torque problems" and expects to find report #39,
which says *"Due to high torque decision to pull out of hole"* — even
though the word "problems" never appears there. Chapter 3's keyword
search can't do this; it needs the exact token "problems" to appear
somewhere, and it has no notion that "high torque... decision to pull
out" describes a problem at all. Semantic search retrieves by *meaning*,
not spelling.
...
Semantic search (this chapter): report #39 ranks highest —
  "Work tight hole at 6,526'... Due to high torque decision to pull
  out of hole" — despite sharing no vocabulary with the query.
```
:::
...
ranks in the top results, despite no keyword overlap with "torque problems."

## Challenge exercise

**Challenge:** Chunk each report into individual `TIME BREAKDOWN` line
items (rather than embedding the whole report as one vector) and re-run
the search. Compare which granularity gives more precise, directly
quotable results — a whole-report embedding blends the packer failure,
the fishing operation, and the drilling narrative in report #49 together;
does line-level chunking separate them more usefully? A reference
solution is in `code/chapter_04/challenge/`.
