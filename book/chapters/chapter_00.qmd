...
![](../figures/diagrams/pipeline_ch00.pdf){width="220" fig-align="center"}
:::
...
`requirements.txt` installs everything needed across the *entire* book in
one pass: Part I's core packages, Part II's heavier retrieval and
chunking dependencies, the notebook tooling, the OCR packages Chapter 6
uses, and the optional companion app's Streamlit dependency. That's
deliberate — one install, no mid-book surprises — but it does mean this
first install is heavier than Part I alone strictly requires. If you only
want to work through Chapters 1–5 for now, the comments inside
`requirements.txt` mark which lines belong to Part II, OCR, the app, or
testing, so you can comment those out and install a lighter subset; just
know you'll need the rest before Chapter 6.
