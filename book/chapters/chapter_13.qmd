...
Farid, the production engineer, asks the question every previous chapter
...
new rows onto the end. Because Chapter 4's embeddings are normalised and
...
(inverse document frequency) and normalises for average document length.
...
Here's the daily loop, end to end, grouped into the four phases the
system moves through. Notice the duplicate check runs first, before
extraction, for the same reason `ingest_report` puts it first in code:
there's no point extracting text from a report you're about to discard.

::: {.content-visible when-format="html"}
::: {.pipeline-diagram}
![](../figures/diagrams/theory_ch13_dailyloop_light.svg){.diagram-light width="420"}
![](../figures/diagrams/theory_ch13_dailyloop_dark.svg){.diagram-dark width="420"}
:::
:::

::: {.content-visible when-format="pdf"}
![](../figures/diagrams/theory_ch13_dailyloop.pdf){width="420" fig-align="center"}
:::
...
def already_ingested(metadata, report):
...
def ingest_report(pdf_path, model, index, metadata):
    ...
    if already_ingested(metadata, report):                  # skip if already added
...
The path check comes before anything else: without it, handing this
function a typo'd filename would fail deep inside `extract_text()`, with
a long, confusing error from Chapter 1's PDF-reading code instead of the
clear message above pointing at the actual mistake. Read top to bottom
after that, this is the daily loop in one function. The
...
are normalised, so the row you append today is bit-for-bit the row a
