...
- Explain why the dense index appends cleanly but the BM25 signal does
  not, and make ingestion idempotent on an append-only index.
...
9's sparse signal scores a term by its rarity across the whole corpus
...
can't append to BM25; you recompute its corpus statistics. That's cheap at
...
**An append-only index has to be idempotent at the door.** Because you
can't easily un-add a row, the moment to prevent a report being indexed
...
```
      ...
      extract text (Ch 1)  ──►  quality gate (Ch 6)  ──► reject → review queue
                  │                     │ pass
                  │            already ingested? (idempotency) ──► yes → skip
                  │                     │ no
        page-aware chunk (Ch 7)  ──►  embed (Ch 4)  ──►  index.add() (Ch 8)
                  │
        re-run the gap check (Ch 10)  ──►  "no report filed yesterday?"
```
...
### Step 2: ingest one report, incrementally and idempotently
...
def ingest_report(pdf_path, model, index, metadata):
    ...
    if any(m["report"] == report for m in metadata):        # idempotent
...
idempotency check comes *first*, because an append-only index can't take
...
every term's inverse document frequency and the corpus's average document
...
is a property of a chunk *relative to the whole corpus*. Anything defined
relative to the whole corpus moves when the corpus moves.
...
Naive idempotency (match on filename) would treat it as new and index it
alongside the stale version, so a query could retrieve both. Extend
...
- Not every signal appends. The dense index does; BM25's corpus statistics
  are recomputed. Know which is which before you deploy.
- An append-only index must be made idempotent at ingestion, because it
  can't cleanly un-add a row after the fact.
...
- [x] Made ingestion idempotent on an append-only index, and re-ran the gap check
:::
