...
kind of check, generalised: for a given term, it compares frequency in a
...
- torque on its own is a weak, non-diagnostic signal — a real driller
  reads a torque jump alongside drag, overpull, and ROP together before
  concluding anything, because torque alone can rise for reasons that
  have nothing to do with a sticking BHA (a formation change, a
  different mud property, a genuinely tighter dogleg). This chapter's
  check is a first-pass filter for *which reports deserve a closer
  read*, not a diagnosis by itself
...
- Genuine cross-well intelligence needs more than one well's worth of
  ...
  structure in the companion pipeline is already organised to support
...
::: {.content-visible when-format="html"}
::: {.pipeline-diagram}
![](../figures/diagrams/theory_ch12_satellites_light.svg){.diagram-light width="420"}
![](../figures/diagrams/theory_ch12_satellites_dark.svg){.diagram-dark width="420"}
:::
:::

::: {.content-visible when-format="pdf"}
![](../figures/diagrams/theory_ch12_satellites.pdf){width="420" fig-align="center"}
:::
