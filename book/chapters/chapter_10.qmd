...
chunk-level index, and read the report and page straight off whichever
chunk matched — never re-typed, never assumed.
...
real reports, real pages, in ranked order.
...
`metadata[row_index]` — the real report and page Chapter 8 recorded for
that exact chunk — and wraps it in a `Citation`. There's no separate
"figure out the source" step: the metadata was attached back when the
chunk was indexed, so reading it is all citing requires.

Run this against the real archive for `"stuck pipe"` and the top result
is:

```
Evidence:
  FORGE-16A-78-32_Drilling_038_2020-11-26.txt page 1
```
...
claim carries a real, live-search-sourced citation — report and page,
never hand-typed — and every silence in the archive gets surfaced
instead of hidden.
:::
