...
Token-based chunking with overlap looks like this, using report #38's
own sentence from the naive-split example above (shown here as whole
words for readability — the real tokens `tiktoken` produces are
sub-word pieces, not words) — each chunk shares a few words with its
neighbour, so no boundary loses context entirely:

::: {.content-visible when-format="html"}
::: {.pipeline-diagram}
![](../figures/diagrams/theory_ch07_chunkoverlap_light.svg){.diagram-light width="380"}
![](../figures/diagrams/theory_ch07_chunkoverlap_dark.svg){.diagram-dark width="380"}
:::
:::

::: {.content-visible when-format="pdf"}
![](../figures/diagrams/theory_ch07_chunkoverlap.pdf){width="380" fig-align="center"}
:::
