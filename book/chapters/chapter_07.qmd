...
- Sketch a segment-aware chunking approach that breaks at natural report
...
Without overlap, a phrase can land exactly on a chunk boundary and get
split between two chunks — neither of which then retrieves the complete
idea. Overlap repeats a few words at each boundary so the full phrase
survives inside at least one chunk. It's the same phrase from report #38 that the naive split cut in half
above — the words in **bold** below are what overlap repeats, so this
time the phrase survives whole in at least one chunk.

::: {.content-visible when-format="html"}
::: {.pipeline-diagram}
![](../figures/diagrams/theory_ch07_chunkoverlap_light.svg){.diagram-light width="640"}
![](../figures/diagrams/theory_ch07_chunkoverlap_dark.svg){.diagram-dark width="640"}
:::
:::

::: {.content-visible when-format="pdf"}
![](../figures/diagrams/theory_ch07_chunkoverlap.pdf){width="640" fig-align="center"}
:::

*Chunk size: 6 words · Overlap: 2 words. Shown as whole words for
readability — the real tokens `tiktoken` produces are sub-word pieces,
not words.*
...
The three checks up front rule out inputs that would otherwise make the
sliding window misbehave silently: a `chunk_tokens` of zero or less can
never produce a chunk at all, a negative `overlap_tokens` doesn't mean
anything, and an `overlap_tokens` that's too large makes `start` stop
moving forward — the loop would then either run forever or repeat the
same chunk endlessly instead of stopping with a clear message about
what went wrong.
...
The empty-`pages` check exists because this function's whole job depends
on those markers being there: text that was never run through Chapter
1's `extract_text()` (or had its markers stripped) has no page to attach
a chunk to, and silently returning an empty list would look like "this
report has no chunks" instead of "this text can't be paginated at all."
