...
::: {.callout-tip title="Engineering Translation: Embedding model vs. language model"}
Chapter 4's **embedding model** and this chapter's **language model** are
easy to blur together — both get called "the model" — but they do
different jobs. The embedding model turns text into GPS-coordinates-for-
meaning so you can *retrieve* the right reports (Chapter 4). The language
model *writes*: given those retrieved reports as evidence, it produces
the answer's actual sentences. One retrieves, the other writes — two
different models, chained together, is what RAG means in practice.
:::
...
small open model on your own hardware. Install it, pull a model, and start
the server:
...
deterministic (the Production Reality note below says why) — but the
grounded fact, the ball that didn't seat, comes back every time, because
it's sitting in the evidence the model was handed.
