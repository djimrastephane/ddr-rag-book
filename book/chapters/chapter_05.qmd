...
Retrieval (Chapter 4) gets you the right passages. What Carlos, the
...
- `llm_call`: any function that takes a prompt string and returns the
  model's answer.
- `top_k`: how many reports to retrieve as evidence, defaulting to 3.
...
memory for it. `host` is configurable too — point it at another machine
on the network running Ollama, or swap the one HTTP call for a hosted
API's to use a cloud model instead. The retrieval and prompt work
above doesn't change either way — only which model reads the evidence.
...
traceable to. This is Part I's capstone artefact — and the known limits
