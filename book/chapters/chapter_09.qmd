...
scale can't unfairly take over the fused result.
...
These specific rankings come from the companion pipeline's real BM25
implementation (`DDR_UTAH_FORGE/src/rag_pdf/retrieval/hybrid_utils.py`),
not from anything bundled in this book — `code/chapter_09/hybrid_search.py`
only implements the RRF fusion math itself, applied here to a hardcoded
toy ranking, so you can't reproduce the BM25-alone or fused numbers above
by running this book's own code.
