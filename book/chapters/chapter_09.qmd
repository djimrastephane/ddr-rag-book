...
Carlos, the completions engineer, is back with the packers question from
...
::: {.callout-tip title="Engineering Translation: Hybrid retrieval"}
**Hybrid retrieval** just means running more than one search method on
the same query and combining their results, instead of betting everything
on a single technique. Here, that's BM25 (below — an exact-match method,
Chapter 3's keyword search made more careful) plus dense embeddings
(Chapter 4's semantic search): two different ways of finding relevant
reports, fused into one ranking.
:::
...
A dense embedding is like summarising the query and each report
...
Compare this to the classic Robertson–Spärck Jones IDF,
...
raise an error or produce `NaN` (a special "not a number" placeholder
floating-point math uses for undefined results like 0/0). The guard
above catches that case explicitly and
...
::: {.callout-tip title="Engineering Translation: defaultdict"}
A plain dictionary raises an error the first time you try to add to a
key it hasn't seen yet — you'd have to check "has this report already
got a score?" before every single addition. `defaultdict(float)` skips
that check: the first time a new report shows up, it's silently handed
a starting score of `0.0` automatically, so `scores[doc_id] += ...`
always works, on the first mention of a report or the fiftieth.
:::
...
The `weights` check exists because `zip(ranked_lists, weights)` doesn't
error on a length mismatch — it just silently stops at whichever list
runs out first, quietly dropping any ranked list past the end of a
too-short `weights` list. That's a fusion bug that would produce a
plausible-looking ranking while silently ignoring a whole retrieval
method, so it's rejected outright instead of being allowed to happen
quietly.
...
The `import` for `rank_bm25` sits inside the `try`, right where it's
used, not at the top of the file — that's what makes the fallback
actually reachable. An import at the top of the file runs the moment
the file loads, before `rank_bm25()` is ever called, so a missing
`rank-bm25` package would stop the whole script immediately instead of
letting this function fall back to the plain term-frequency count.
...
— it matches literal words, same as Chapter 3's keyword search — so by
that chapter's Field Note it belongs on the *expanded* text, while the
dense signal uses the raw. This exercise runs both over the same raw
