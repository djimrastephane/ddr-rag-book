...
def test_citations_from_search_reads_the_real_metadata_not_the_score():
    ...
    metadata = [
        {"report": "report_038.pdf", "page": 1},
        {"report": "report_039.pdf", "page": 1},
    ]
    ...
    assert citations == [
        Citation(report="report_039.pdf", page=1),
        Citation(report="report_038.pdf", page=1),
    ]
...
def test_citations_from_search_cites_the_real_stuck_pipe_page(extracted_sample_text_dir):
    ...
    assert "FORGE-16A-78-32_Drilling_038_2020-11-26.txt page 1" in evidence
